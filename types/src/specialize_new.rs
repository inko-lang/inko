use crate::{
    Block, ClosureId, Database, InternedTypeArguments, SpecializationKey,
    TraitId, TraitInstance, TypeArguments, TypeEnum, TypeId, TypeInstance,
    TypeRef,
};
...
impl<'a> TypeSpecializer<'a> {
    pub fn new(
        ...
        TypeSpecializer { db, interned, type_arguments, types, self_type }
    }
    ...
    pub fn specialize(&mut self, value: TypeRef) -> TypeRef {
        match value {
            ...
            | TypeRef::Any(
                TypeEnum::TypeParameter(pid)
                | TypeEnum::RigidTypeParameter(pid),
            ) => self.type_arguments.get(pid).unwrap(),
            TypeRef::Uni(
                TypeEnum::TypeParameter(pid)
                | TypeEnum::RigidTypeParameter(pid),
            ) => self.type_arguments.get(pid).unwrap().as_uni(self.db),
            TypeRef::Ref(
                TypeEnum::TypeParameter(pid)
                | TypeEnum::RigidTypeParameter(pid),
            ) => self.type_arguments.get(pid).unwrap().as_ref(self.db),
            TypeRef::Mut(
                TypeEnum::TypeParameter(pid)
                | TypeEnum::RigidTypeParameter(pid),
            ) => self.type_arguments.get(pid).unwrap().as_mut(self.db),
            TypeRef::UniRef(
                TypeEnum::TypeParameter(pid)
                | TypeEnum::RigidTypeParameter(pid),
            ) => self.type_arguments.get(pid).unwrap().as_uni_ref(self.db),
            TypeRef::UniMut(
                TypeEnum::TypeParameter(pid)
                | TypeEnum::RigidTypeParameter(pid),
            ) => self.type_arguments.get(pid).unwrap().as_uni_mut(self.db),
            ...
            TypeRef::Placeholder(t) => {
                t.value(self.db).map_or(value, |v| self.specialize(v))
            }
    ...
    fn specialize_generic_instance(
        ...
        // Update the type arguments so that we actually use them when interning
        // them.
        ins.replace_type_arguments(self.db, args);
        ...
        // TODO: just return a regular type instance here
        ins.with_type_id(new)
    }
    ...
    fn specialize_closure_type(&mut self, old: ClosureId) -> ClosureId {
        ...
        for arg in old.arguments(self.db) {
            let name = arg.name;
            let loc = arg.variable.location(self.db);
            let var_typ = self.specialize(arg.variable.value_type(self.db));
            let arg_typ = self.specialize(arg.value_type);

            new.new_argument(self.db, name, var_typ, arg_typ, loc);
        }

        let ret = self.specialize(old.return_type(self.db));

        // The same closure signature used in different places produces a
        // different ClosureId. This means we don't have a stable location for
        // tracking specializations, and so we simply always create a new type
        // for each closure type.
        new.set_return_type(self.db, ret);
    ...
    fn specialize_closure_literal(
        ...
        let key = SpecializationKey::for_closure(stype, self.type_arguments);
        let typ = ins.instance_of();
        let new = typ
            .specializations(self.db)
            .get(&key)
    ...
    fn specialize_type(
        ...
        new.set_specialization_source(self.db, old);
        new.set_type_arguments(self.db, instance.type_arguments);
        ...
        let targs = instance.type_arguments(self.db).unwrap().clone();
    ...
    fn specialize_trait_instance(
        ...
        let mut ins = ins.interned(self.db, self.interned);
        let key = SpecializationKey::new(ins.type_arguments(self.db).unwrap());
        let new = typ
            .specializations(self.db)
            .get(&key)
...
mod test {
    ...
    use crate::test::{
        ...
        mutable_uni, new_enum_type, new_parameter, new_trait, new_type, owned,
        parameter, placeholder, pointer, trait_instance, uni,
    };
    ...
    fn test_specialize_generic_type() {
        ...
        assert_eq!(format_type(&db, new1), "Array[Int]");
        assert_eq!(format_type(&db, new2), "Array[Int]");
        assert_eq!(format_type(&db, new3), "Array[Int]");
        assert_eq!(format_type(&db, new4), "Array[String]");
