...
impl TypeArguments {
    ...
    pub fn values<'a>(&'a self) -> impl Iterator<Item = TypeRef> + 'a {
        ...
        while let Some(typ) = stack.pop() {
            ...
            let (val, args) = match typ {
                TypeRef::Owned(t) | TypeRef::Any(t) => {
                    match t.prepare_interning(db) {
                        (t, a) => (TypeRef::Owned(t), a),
                    }
                }
                TypeRef::Uni(t) => match t.prepare_interning(db) {
                    (t, a) => (TypeRef::Uni(t), a),
                },
                TypeRef::Ref(t) => match t.prepare_interning(db) {
                    (t, a) => (TypeRef::Ref(t), a),
                },
                TypeRef::Mut(t) => match t.prepare_interning(db) {
                    (t, a) => (TypeRef::Mut(t), a),
                },
                TypeRef::UniRef(t) => match t.prepare_interning(db) {
                    (t, a) => (TypeRef::UniRef(t), a),
                },
                TypeRef::UniMut(t) => match t.prepare_interning(db) {
                    (t, a) => (TypeRef::UniMut(t), a),
                },
                TypeRef::Pointer(t) => match t.prepare_interning(db) {
                    (t, a) => (TypeRef::Pointer(t), a),
                },
                _ => (typ, None),
            };
