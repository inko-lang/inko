...
use crate::{
    ...
    TypeBounds, TypeId, TypeParameterId, TypePlaceholder, TypeRef,
};
...
impl<'a> TypeResolver<'a> {
    ...
    pub fn resolve(&mut self, value: TypeRef) -> TypeRef {
        ...
        let resolved = match value {
            ...
            // type.
            TypeRef::Placeholder(id) => match id.resolve(self.db) {
                TypeRef::Unknown => value,
        ...
        match id {
            ...
            TypeId::TypeParameter(pid) => {
                ...
                match self.resolve_type_parameter(pid) {
                    ...
                    _ => Either::Right(TypeRef::Placeholder(
                        TypePlaceholder::alloc(self.db, Some(pid)),
                    )),
                }
            }
            TypeId::RigidTypeParameter(pid) => {
                ...
                match self.resolve_type_parameter(pid) {
                    Some(val) => Either::Right(val),
                    None => Either::Left(TypeId::RigidTypeParameter(pid)),
                }
            }
            TypeId::Closure(id) => {
                ...
                new.throw_type = self.resolve(new.throw_type);
...
mod tests {
    ...
    fn resolve_immutable(
        ...
        let mut resolver = TypeResolver::new(db, type_arguments, bounds);

        resolver.immutable = true;
    ...
    fn test_closure() {
        ...
        fun.set_throw_type(&mut db, owned(parameter(param)));
        fun.new_argument(
            &mut db,
            "a".to_string(),
            owned(rigid(param)),
        ...
        assert_eq!(output.throw_type(&db), TypeRef::Any);
