...
use types::specialize::TypeSpecializer;
use types::{
...
fn specialize_constants(
    ...
    while let Some((id, cons)) = stack.pop() {
        ...
        let new = TypeSpecializer::new(db, interned, &targs, &mut types, stype)
            .specialize(*old);
...
impl Dynamic {
    ...
    fn calls_for<'a>(
        &'a self,
        trait_id: TraitId,
    ) -> impl Iterator<Item = DynamicCall> + 'a {
...
impl<'a, 'b> Specialize<'a, 'b> {
    ...
    fn call_dynamic(
        ...
        method: MethodId,
        type_arguments: Option<usize>,
        all_type_arguments: &Vec<TypeArguments>,
        dynamic: &mut Dynamic,
    ...
    fn specialize_type_arguments(&mut self, arguments: &mut TypeArguments) {
        // The order of merging argument is important: the "local" arguments
        // must come first because they may contain type parameters that are
        // assigned types by the type arguments of the surrounding method.
        let mut all = arguments.clone();

        self.type_arguments.copy_into(&mut all);

        for idx in 0..(all.len()) {
            let par = all.get_parameter_at(idx).unwrap();
            let old = all.get_recursive(&self.state.db, par).unwrap();
            let new = self.specialize_with(old, self.self_type, &all);
