...
impl<'a> GenerateInlineMethods<'a> {
    ...
    fn enum_class(&mut self) {
        // TODO: reuse logic as much as possible?
        // TODO: reuse somehow with GenerateDropper
        self.enum_increment();
        self.enum_decrement();
    }
    ...
    fn enum_increment(&mut self) {
        ...
        let rec = lower.self_register;
        let cons = cls.constructors(lower.db());
        let mut blocks = Vec::new();
        let before = lower.current_block;
        let after = lower.add_block();
        let enum_fields = cls.enum_fields(lower.db());
        let tag_field =
            cls.field_by_index(lower.db(), types::ENUM_TAG_INDEX).unwrap();
        let tag = lower.new_register(TypeRef::int());

        for con in cons {
            let block = lower.add_current_block();

            lower.add_edge(before, block);
            ...
            lower.current_block_mut().goto(after, loc);
            lower.add_edge(lower.current_block, after);
            blocks.push(block);
        }

        lower.block_mut(before).get_field(tag, rec, cls, tag_field, loc);
        lower.block_mut(before).switch(tag, blocks, loc);
        lower.current_block = after;
