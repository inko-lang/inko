...
impl Registers {
    ...
    pub(crate) fn alloc(&mut self, value_type: types::TypeRef) -> RegisterId {
        ...
        self.values.push(Register { value_type, argument: false });
        RegisterId(id)
    }
    ...
    pub(crate) fn is_argument(&self, register: RegisterId) -> bool {
        self.get(register).argument
    }

    pub(crate) fn set_argument(&mut self, register: RegisterId) {
        self.get_mut(register).argument = true;
    }
...
impl Method {
    ...
    pub(crate) fn add_argument(&mut self, register: RegisterId) {
        self.registers.set_argument(register);
        self.arguments.push(register);
    }
    ...
    fn apply_local_optimizations(
        ...
        // This runs _after_ merging redundant moves as it simplifies the
        // optimization logic (e.g. when propagating state across registers).
        refs::remove_redundant_reference_counts(db, self);
    }
