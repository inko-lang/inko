...
impl Method {
    pub(crate) fn new(hash: u64, code: extern "system" fn()) -> Method {
        Method { hash, code }
    }
}
...
impl Int {
    ...
    pub(crate) unsafe fn read(ptr: *const Int) -> i64 {
        if is_tagged_int(ptr) {
            ptr as i64 >> INT_SHIFT
        } else {
            (*ptr).value
        }
    }
...
impl String {
    ...
    pub(crate) unsafe fn read_as_c_char<'a>(
        ptr: *const String,
    ) -> *const c_char {
        (*ptr).value.as_c_char_pointer()
    }
...
mod tests {
    ...
    fn test_tagged_int() {
        unsafe {
            assert_eq!(Int::read(tagged_int(3)), 3);
            assert_eq!(Int::read(tagged_int(0)), 0);
            assert_eq!(Int::read(tagged_int(-3)), -3);
            assert!(is_tagged_int(tagged_int(3)));
        }
    }
    ...
    fn test_int_read() {
        let state = setup();
        let tagged = Int::new(state.int_class, 42);
        let max = Int::new(state.int_class, i64::MAX);
        let min = Int::new(state.int_class, i64::MIN);
