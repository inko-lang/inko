...
use crate::mem::{
    allocate, free, ClassPointer, Header, Int, String as InkoString,
};
...
///
/// Frames are statically allocated and managed by the generated code, hence
/// this type doesn't implement `Drop`.
#[repr(C)]
pub struct StackFrame {
    pub name: *const InkoString,
    pub path: *const InkoString,
    pub line: *const Int,
}
...
impl Message {
    pub(crate) fn new(method: NativeAsyncMethod, length: u8) -> OwnedMessage {
        unsafe {
...
pub struct Process {
    ...
    /// The call stack.
    ///
    /// The most recent entry comes last. The values are pointers to statically
    /// allocated stack frames, managed by the generated/native code.
    pub(crate) call_stack: Vec<*const StackFrame>,
...
impl Process {
    ...
    pub(crate) fn main(
        ...
        let message = Message::new(method, 0);
...
mod tests {
    ...
    fn test_type_sizes() {
        ...
        assert_eq!(size_of::<Mutex<ProcessState>>(), 56);
    ...
    fn test_field_offsets() {
        ...
        assert_eq!(offset_of!(proc, fields), 136);
    }
    ...
    #[test]
    fn test_message_new() {
        let message = Message::new(method, 2);
    ...
    fn test_process_next_task_with_existing_message() {
        ...
        let stack = Stack::new(32);
        let mut process = OwnedProcess::new(Process::alloc(*proc_class, stack));
        let msg1 = Message::new(method, 0);
        let msg2 = Message::new(method, 0);
