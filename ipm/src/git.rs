...
impl Repository {
    pub(crate) fn open(path: &Path) -> Result<Self, Error> {
        if path.is_dir() {
            ...
            fail!("The Git repository at {} doesn't exist", path.display())
        }
    }

    pub(crate) fn clone(url: &str, path: &Path) -> Result<Self, Error> {
        run("clone", None, &[OsStr::new(url), path.as_os_str()])
            .map_err(|err| error!("Failed to clone {}: {}", url, err))?;

        Ok(Self { path: path.to_path_buf() })
    }

    pub(crate) fn fetch(&mut self) -> Result<(), Error> {
        run(
            "fetch",
        ...
        .map_err(|err| {
            error!("Failed to update {}: {}", self.path.display(), err)
        })?;
    ...
    pub(crate) fn checkout(&self, name: &str) -> Result<(), Error> {
...
fn run(
    ...
    let child = cmd
        .spawn()
        .map_err(|err| error!("Failed to spawn 'git {}': {}", command, err))?;
    let output = child.wait_with_output().map_err(|err| error!("{}", err))?;
...
mod tests {
    ...
    fn test_repository_clone() {
        let temp = temp_dir().join("ipm-test_repository_clone");
    ...
    fn test_repository_fetch() {
        let temp = temp_dir().join("ipm-test_repository_fetch");
        let mut repo =
    ...
    fn test_repository_tag() {
        let temp = temp_dir().join("ipm-test_repository_tag");
        let mut repo =
            Repository::clone(source_dir().to_str().unwrap(), &temp).unwrap();
    ...
    fn test_repository_checkout() {
        let temp = temp_dir().join("ipm-test_repository_checkout");
        let mut repo =
            Repository::clone(source_dir().to_str().unwrap(), &temp).unwrap();
    ...
    fn test_repository_version_tag_names() {
        let temp = temp_dir().join("ipm-test_repository_version_tag_names");
        let mut repo =
            Repository::clone(source_dir().to_str().unwrap(), &temp).unwrap();
